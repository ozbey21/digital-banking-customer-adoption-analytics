...
PostgreSQL KPI and index view script

Creates calculated KPI views from the seeded core tables.
*/
...
CREATE VIEW digital_adoption_index_2025 AS
WITH growth AS (
    SELECT
        bank_name,
...
SELECT
    ...
    'Directional project-specific index based on public disclosures; not an official regulatory ranking.' AS methodology_note
FROM final_index;

CREATE VIEW digital_adoption_index_2025_equal_weight AS
SELECT
    bank_name,
    reported_digital_penetration_pct,
    digital_customers_per_branch,
    ...
    'Equal-weight sensitivity check. Used to test whether the index result is highly sensitive to weighting assumptions.' AS methodology_note
FROM digital_adoption_index_2025;
